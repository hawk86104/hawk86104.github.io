import{importShared as s,mergeGeometries as T}from"./3d-tiles-renderer.NEVve1-v1778574646008.js";import{useTres as R,useLoop as B}from"./index.Bs6ST5xA1778574646008.js";import{useTexture as E}from"./index.vue_vue_type_script_setup_true_lang.CI_dVrQ71778574646008.js";import"./index.vue_vue_type_script_setup_true_lang.CIGqmYkD1778574646008.js";var F=`uniform float c;
uniform float p;
uniform float uTime;
...
}`;const{defineComponent:$}=await s("vue"),{createElementVNode:x,normalizeProps:A,guardReactiveProps:D,openBlock:k,createElementBlock:O}=await s("vue"),f=await s("three"),{ref:V,watchEffect:G,onMounted:N}=await s("vue"),ne=$({__name:"xRayEffect",props:{model:{},color:{default:"#84ccff"},opacity:{default:1}},setup(v){const e=v,a=V(),i=[];e.model.traverse(n=>{n instanceof f.Mesh&&(n.geometry.verticesNeedUpdate=!0,i.push(n.geometry))});const t={uniforms:{c:{type:"f",value:1.11},p:{type:"f",value:1},glowColor:{type:"c",value:new f.Color(e.color)},lightningTexture:{type:"t",value:null},offsetY:{type:"f",value:.1},uTime:{type:"f",value:0},uOpacity:{type:"f",value:e.opacity}},vertexShader:F,fragmentShader:U,side:f.DoubleSide,blending:f.AdditiveBlending,depthWrite:!1};N(async()=>{const n=await E("./plugins/medical/image/brainXRayLight.png");t.uniforms.lightningTexture.value=n,console.log(n)}),t.uniforms.offsetY.value=Math.sin(5);const{camera:b}=R(),{onBeforeRender:c}=B();return c(({delta:n})=>{b.value?.position&&a.value&&(t.uniforms.uTime.value+=n)}),G(()=>{a.value&&(a.value.geometry.dispose(),a.value.geometry=T(i)),e.color&&(t.uniforms.glowColor.value=new f.Color(e.color)),e.opacity&&(t.uniforms.uOpacity.value=e.opacity)}),(n,m)=>(k(),O("TresMesh",{ref_key:"TresMeshRef",ref:a,"render-order":9},[m[0]||(m[0]=x("TresBufferGeometry",null,null,-1)),x("TresShaderMaterial",A(D(t)),null,16)],512))}});var j=`uniform float p;
uniform float uTime;
