import { importShared, zr, Fs, gl, useDevicePixelRatio, _l } from './index.BxB45aCK1767105581793.js';
import { EffectComposer, ChromaticAberrationEffect, RenderPass, BloomEffect, GridEffect, OutlineEffect, NormalPass, DepthDownsamplingPass, EffectPass } from './index.s6u766IF1767105581793.js';
 ...
 */
const {defineComponent:c,shallowRef:W,provide:je,computed:L,watch:m,onUnmounted:N,renderSlot:Ve,inject:K,watchEffect:S,nextTick:fe,toRaw:me} = await importShared('vue');
const {HalfFloatType:C,Uniform:M,Vector2:d,Vector3:F,Mesh:Ye,SphereGeometry:at,MeshBasicMaterial:ke,OrthographicCamera:rt,BufferGeometry:it,Float32BufferAttribute:Oe,ShaderMaterial:y,UniformsUtils:P,WebGLRenderTarget:D,NoBlending:nt,Clock:lt,Color:Q,MathUtils:A,DataTexture:ct,RedFormat:ut,FloatType:dt,MeshNormalMaterial:ft,NearestFilter:B,DepthTexture:ht,Vector4:pt,RawShaderMaterial:mt,ColorManagement:vt,SRGBTransfer:gt,LinearToneMapping:xt,ReinhardToneMapping:bt,CineonToneMapping:St,ACESFilmicToneMapping:wt,AgXToneMapping:At,NeutralToneMapping:yt,CustomToneMapping:Mt,Texture:Be,LinearFilter:Tt,AdditiveBlending:Ct} = await importShared('three');
class Re {
  /**
...
const $ = Symbol("effectComposerPmndrs"), _t = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t, emit: e }) {
    const s = r, o = e, { scene: a, camera: i, renderer: n, sizes: l } = zr(), u = W(null);
    let h = null, x = null;
    je($, u), t({ composer: u });
    const w = () => {
      u.value && (x = new NormalPass(a.value, i.activeCamera.value), x.enabled = false, u.value.addPass(x), s.resolutionScale !== void 0 && Re.isWebGL2Available() && (h = new DepthDownsamplingPass({
        normalBuffer: x.texture,
        resolutionScale: s.resolutionScale
      }), h.enabled = false, u.value.addPass(h)));
    }, U = L(() => {
      const v = new EffectComposer(), b = {
      ...
      !n.instance && !a.value && !i.activeCamera.value || (u.value?.dispose(), u.value = new EffectComposer(n.instance, U.value), u.value.addPass(new RenderPass(a.value, i.activeCamera.value)), s.disableNormalPass || w());
    };
  ...
  const s = K($), o = W(null), a = W(null), { scene: i, camera: n, invalidate: l } = Fs();
  m(t, () => l());
  const u = () => {
    o.value && s?.value?.removePass(o.value), a.value?.dispose(), o.value?.dispose();
  }, h = (w) => {
    !n.value || !s?.value || !i.value || (a.value = r(), o.value = new EffectPass(n.value, a.value), s.value.addPass(o.value, w));
  };
  e && m(
    () => e.map((w) => t[w]),
    () => {
  ...
  setup(r, { expose: t }) {
    const e = r, { pass: s, effect: o } = f(() => new BloomEffect(e), e, ["mipmapBlur"]);
  ...
  setup(r, { expose: t }) {
    const e = r, s = (u) => u !== void 0 ? gl(u).getHex() : void 0, { camera: o, scene: a } = Fs(), { pass: i, effect: n } = f(
      () => new OutlineEffect(
        a.value,
        o.value,
        {
    ...
    const l = L(() => ({
      hiddenEdgeColor: e.hiddenEdgeColor ? gl(e.hiddenEdgeColor) : void 0,
      visibleEdgeColor: e.visibleEdgeColor ? gl(e.visibleEdgeColor) : void 0
    }));
    return p(
      [
        [() => e.blendFunction, "blendMode.blendFunction"],
...
const to = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    const e = r, s = new ChromaticAberrationEffect(), { pass: o, effect: a } = f(() => new ChromaticAberrationEffect({
      ...e,
...
const vo = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    const e = r, { pass: s, effect: o } = f(() => new GridEffect(e), e);
    return t({ pass: s, effect: o }), p(
      [
        [() => e.blendFunction, "blendMode.blendFunction"],
        [() => e.scale, "scale"],
...
class Nt {
  ...
  constructor(t, e) {
    ...
    this.renderTarget1 = e, this.renderTarget2 = e.clone(), this.renderTarget2.texture.name = "EffectComposer.rt2", this.writeBuffer = this.renderTarget1, this.readBuffer = this.renderTarget2, this.renderToScreen = true, this.passes = [], this.copyPass = new Bt(k), this.copyPass.material.blending = nt, this.clock = new lt();
  }
  /**
  ...
  render(t) {
    ...
    for (let o = 0, a = this.passes.length; o < a; o++) {
      ...
      if (i.enabled !== false) {
        ...
        He !== void 0 && (i instanceof He ? s = true : i instanceof Rt && (s = false));
      }
    }
...
class zt extends E {
  ...
  constructor(t, e, s = null, o = null, a = null) {
    super(), this.scene = t, this.camera = e, this.overrideMaterial = s, this.clearColor = o, this.clearAlpha = a, this.clear = true, this.clearDepth = false, this.needsSwap = false, this._oldClearColor = new Q();
  }
  /**
...
const Ke = Symbol("effectComposerThree"), yo = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    ...
    const { renderer: o, sizes: a, scene: i, camera: n } = zr();
    S(() => {
    ...
    const { pixelRatio: u } = useDevicePixelRatio();
    return S(() => {
  ...
  const s = K(Ke), o = W(r()), { sizes: a, invalidate: i } = Fs();
  t && m(t, () => i());
  const n = () => {
...
class Ie extends E {
  ...
  constructor(t = 64) {
    ...
    }), this.goWild = false, this._heightMap = this._generateHeightmap(t), this.uniforms.tDisp.value = this.heightMap, this._fsQuad = new O(this.material), this._curF = 0, this._randX = 0, this._generateTrigger();
  }
  /**
...
const Mo = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    const e = r, { pass: s } = z(() => new Ie(e.dtSize), e, ["dtSize"]), { invalidate: o } = Fs();
    t({ pass: s });
    const { onBeforeRender: a } = _l();
    return a(() => o()), p(
      [[() => e.goWild, "goWild"]],
      s,
      () => new Ie()
...
class Gt extends E {
  ...
  constructor() {
    ...
    }, this._areaTexture = new Be(), this._areaTexture.name = "SMAAPass.area", this._areaTexture.image = e, this._areaTexture.minFilter = Tt, this._areaTexture.generateMipmaps = false, this._areaTexture.flipY = false;
    const s = new Image();
    s.src = this._getSearchTexture(), s.onload = function() {
      t._searchTexture.needsUpdate = true;
    }, this._searchTexture = new Be(), this._searchTexture.name = "SMAAPass.search", this._searchTexture.image = s, this._searchTexture.magFilter = B, this._searchTexture.minFilter = B, this._searchTexture.generateMipmaps = false, this._searchTexture.flipY = false, this._uniformsEdges = P.clone(V.uniforms), this._materialEdges = new y({
...
const Po = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    const e = r, { sizes: s } = zr(), { pixelRatio: o } = useDevicePixelRatio(), a = L(() => e.width ?? s.width.value * o.value), i = L(() => e.height ?? s.height.value * o.value), { pass: n } = z(() => new Gt(), e);
    return t({ pass: n }), S(() => {
...
class R extends E {
  ...
  constructor(t, e = 1, s, o) {
    super(), this.strength = e, this.radius = s, this.threshold = o, this.resolution = t !== void 0 ? new d(t.x, t.y) : new d(256, 256), this.clearColor = new Q(0, 0, 0), this.needsSwap = false, this.renderTargetsHorizontal = [], this.renderTargetsVertical = [], this.nMips = 5;
    let a = Math.round(this.resolution.x / 2), i = Math.round(this.resolution.y / 2);
    this.renderTargetBright = new D(a, i, { type: C }), this.renderTargetBright.texture.name = "UnrealBloomPass.bright", this.renderTargetBright.texture.generateMipmaps = false;
    for (let h = 0; h < this.nMips; h++) {
      const x = new D(a, i, { type: C });
      x.texture.name = "UnrealBloomPass.h" + h, x.texture.generateMipmaps = false, this.renderTargetsHorizontal.push(x);
      const w = new D(a, i, { type: C });
      w.texture.name = "UnrealBloomPass.v" + h, w.texture.generateMipmaps = false, this.renderTargetsVertical.push(w), a = Math.round(a / 2), i = Math.round(i / 2);
    }
    const n = Je;
...
const Do = /* @__PURE__ */ c({
  ...
  setup(r, { expose: t }) {
    const e = r, { sizes: s } = zr(), { pass: o } = z(() => new R(
...
export { Do, Eo, Kt, Mo, Po, Yt, _t, to, vo, yo };
//# sourceMappingURL=tres-post-processing.CBYUVLxv1767105581793.js.map
