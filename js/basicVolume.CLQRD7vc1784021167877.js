import{importShared as a}from"./3d-tiles-renderer.DLbEVYVf1784021167877.js";import{useTres as P,useLoop as C,OrbitControls_default as T,_export_sfc as F}from"./index.BjVPg-2F1784021167877.js";import{Pane as M}from"./tweakpane.BQRZXf8M1784021167877.js";var R=`precision mediump float;
...
void main() {
  ...
  gl_FragColor = color;
}`;const{defineComponent:E}=await a("vue"),{createElementVNode:y,unref:S,openBlock:L,createElementBlock:N}=await a("vue"),$=["rotation-x"],A=["uniforms","vertexShader","fragmentShader"],{ref:b,reactive:j,onMounted:W,watch:U}=await a("vue"),o=await a("three"),l=41,G=E({__name:"basicVolumeRendering",setup(k){const v=window.innerWidth,_=window.innerHeight,g=b(null),n=b(null),d=()=>{fetch("/plugins/volumeRendering/image/nucleon_41x41x41_uint8.raw").then(t=>t.arrayBuffer()).then(t=>{const e=new Uint8Array(t),h=new Uint8Array(l*l*l);for(let m=0;m<e.length;m++)h[m]=e[m];n.value=h}).catch(t=>{console.error("Error fetching volume data:",t)})},f=new o.Clock;f.start();const x=new M().addFolder({title:"Display Settings"}),c=x.addFolder({title:"Cross Section Settings"}),u=new o.Vector3(.5,.5,.5);c.addBinding(u,"x",{label:"X",min:.02,max:.5,step:.02}),c.addBinding(u,"y",{label:"Y",min:.02,max:.5,step:.02}),c.addBinding(u,"z",{label:"Z",min:.02,max:.5,step:.02}),c.expanded=!0;const{camera:D}=P(),r=j({u_camera:{value:D.value?.position},u_resolution:{value:new o.Vector3(v,_,1)},u_dt:{value:.004},u_time:{value:0},u_crossSectionSize:{value:u},u_color:{value:1},u_volume:{value:null},u_isoValue:{value:.2},u_alphaVal:{value:.2}}),s=x.addFolder({title:"Algorithm Settings"});s.addBinding(r.u_dt,"value",{label:"dt",min:4e-4,max:.016,step:2e-4}),s.addBinding(r.u_color,"value",{label:"color",min:1,max:3,step:1}),s.addBinding(r.u_alphaVal,"value",{label:"alphaVal",min:.01,max:1,step:.01}),s.addBinding(r.u_isoValue,"value",{label:"isoValue",min:0,max:1,step:.04}),U(n,t=>{if(t){const e=new o.Data3DTexture(t,l,l,l);e.format=o.RedFormat,e.minFilter=o.LinearFilter,e.magFilter=o.LinearFilter,e.wrapT=o.RepeatWrapping,e.needsUpdate=!0,r.u_volume.value=e}});const{onBeforeRender:B}=C();return B(()=>{r.u_time.value=f.getElapsedTime()}),W(()=>{d()}),(t,e)=>(L(),N("TresMesh",{ref_key:"meshRef",ref:g,"rotation-x":Math.PI/-2},[e[0]||(e[0]=y("TresSphereGeometry",{args:[1,16,16]},null,-1)),y("TresShaderMaterial",{uniforms:r,vertexShader:S(R),fragmentShader:S(O)},null,8,A)],8,$))}}),{defineComponent:H}=await a("vue"),{createElementVNode:i,createTextVNode:I,unref:Z,normalizeProps:X,guardReactiveProps:Y,createVNode:p,Suspense:q,withCtx:w,openBlock:z,createBlock:J,resolveComponent:K,mergeProps:Q,Fragment:ee,createElementBlock:ne}=await a("vue"),{reactive:V}=await a("vue"),te=H({__name:"basicVolume",setup(k){const v=V({windowSize:!0,alpha:!0,antialias:!0,clearAlpha:0,disableRender:!0}),_=V({enableDamping:!0,enableZoom:!0,enablePan:!0,enableRotate:!0,makeDefault:!0});return(g,n)=>{const d=K("TresCanvas");return z(),ne(ee,null,[n[3]||(n[3]=i("div",{class:"info"},[i("a",{href:"https://klacansky.com/open-scivis-datasets/skull/skull_256x256x256_uint8.raw",target:"_blank"}," https://klacansky.com/open-scivis-datasets/skull/skull_256x256x256_uint8.raw "),i("br"),I(" 请自行下载数据集, 复制到 /plugins/volumeRendering/image/skull_256x256x256_uint8.raw 并更改文件请求指向和 dim 大小 ")],-1)),p(d,Q({clearColor:"#201919"},v),{default:w(()=>[n[0]||(n[0]=i("TresPerspectiveCamera",{fov:75,near:.001,far:1e3,position:[-1,.4,-1],"look-at":[0,0,0],up:[0,1,0]},null,-1)),n[1]||(n[1]=i("TresAmbientLight",{intensity:2},null,-1)),p(Z(T),X(Y(_)),null,16),(z(),J(q,null,{default:w(()=>[p(G,{position:[0,0,0]})]),_:1})),n[2]||(n[2]=i("TresGridHelper",{args:[50,50],position:[0,-5,0]},null,-1))]),_:1},16)],64)}}}),ie=F(te,[["__scopeId","data-v-08c577c7"]]);export{ie as default};
