...
const THREE$1$1 = await importShared('three');
...
class ParticleState {
    ...
    init() {
        ...
        this.lifetime = 1.0;
        this.age = 0.0;
        this.sequenceElement = new THREE$1$1.Vector4();
        this.position = new THREE$1$1.Vector3();
        this.velocity = new THREE$1$1.Vector3();
        this.acceleration = new THREE$1$1.Vector3();
        this.normal = new THREE$1$1.Vector3();
        this.rotation = 0.0;
        this.rotationalSpeed = 0.0;
        this.size = new THREE$1$1.Vector2();
        this.color = new THREE$1$1.Color();
        this.alpha = 1.0;
        this.initialSize = new THREE$1$1.Vector2();
        this.initialColor = new THREE$1$1.Color();
...
class ParticleStateArray {
    ...
    computeBoundingBox = function() {
        ...
        const tempPos = new THREE$1$1.Vector3();

        return function(outBox = new THREE$1$1.Box3(), positionTransform = null) {
    ...
    computeBoundingSphere = function() {
        ...
        const tempCenter = new THREE$1$1.Vector3();
        const tempVector = new THREE$1$1.Vector3();
        const tempPos = new THREE$1$1.Vector3();

        return function(outSphere = new THREE$1$1.Sphere(), positionTransform = null) {
            let radius = 0;
            for (let i = 0; i < this.activeParticleCount; i++) {
                const particleState = this.getState(i);
                let pos = particleState.position;
...
class BaseParticleStateOperator extends ParticleStateOperator {
    ...
    constructor() {
        super();
        this.timeScaledVelocity = new THREE$1$1.Vector3();
        this.timeScaledAcceleration = new THREE$1$1.Vector3();
        this.stateAcceleration = new THREE$1$1.Vector3();
        this.stateVelocity = new THREE$1$1.Vector3();
    }
...
class ParticleSystem {
    ...
    constructor(owner, particleSystemRenderer) {
        ...
        this.boundingBox = new THREE$1$1.Box3();
        this.boundingSphere = new THREE$1$1.Sphere();
    }
    ...
    updateBounds = function() {
        ...
        const tempMatrix4 = new THREE$1$1.Matrix4();
    ...
    static fromJSON(json, jsonTypeStore, threeRenderer) {
        ...
        const rootObject = new THREE$1$1.Object3D();
...
class ParticleStateAttributeArray extends ParticleStateArray {
    ...
    allocate(particleCount) {
        ...
        const createAttributeBuffer = (bufferArray, componentCount) => {
            if (this.instanced) {
                return new THREE$1$1.InstancedBufferAttribute(bufferArray, componentCount)
            } 
                return new THREE$1$1.BufferAttribute(bufferArray, componentCount)
        ...
        if (this.instanced) {
            const baseGeometry = new THREE$1$1.BufferGeometry();

            const basePositionsArray = new Float32Array(18);
            this.basePositions = new THREE$1$1.BufferAttribute(basePositionsArray, 3);
            baseGeometry.setAttribute('position', this.basePositions);
            this.basePositions.needsUpdate = true;
            this.basePositions.setXYZ(0, -1, 1.0, 0.0);
            this.basePositions.setXYZ(1, -1, -1, 0.0);
            this.basePositions.setXYZ(2, 1.0, 1.0, 0.0);
            this.basePositions.setXYZ(3, -1, -1, 0.0);
            this.basePositions.setXYZ(4, 1.0, -1, 0.0);
            this.basePositions.setXYZ(5, 1.0, 1.0, 0.0);

            const baseUVsArray = new Float32Array(12);
            this.baseUVs = new THREE$1$1.BufferAttribute(baseUVsArray, 2);
            ...
            this.customIndexes = new THREE$1$1.BufferAttribute(customIndexesArray, 1);
            ...
            this.geometry = new THREE$1$1.InstancedBufferGeometry().copy(baseGeometry);
            this.geometry.instanceCount = 0;
        } else {
            this.geometry = new THREE$1$1.BufferGeometry();
...
class AnimatedSpriteRenderer extends Renderer {
    ...
    constructor(instanced, atlas, interpolateAtlasFrames = false,
                blending = THREE$1$1.NormalBlending, calculateBoundingSphereFromBox = true, renderOrder) {
        super();
        this.instanced = instanced;
    ...
    updateMeshBounds = function() {
        ...
        return function() {
            ...
            if (this.calculateBoundingSphereFromBox) {
                if (!geometry.boundingBox) geometry.boundingBox = new THREE$1$1.Box3();
                geometry.boundingBox.copy(this.boundingBox);
                if (!geometry.boundingSphere) {
                    geometry.boundingSphere = new THREE$1$1.Sphere();
                }
                ...
                if (!geometry.boundingSphere) geometry.boundingSphere = new THREE$1$1.Sphere();
    ...
    init(particleCount) {
        if (super.init(particleCount)) {
            ...
            this.mesh = new THREE$1$1.Mesh(this.particleStateArray.getGeometry(), this.material);
    ...
    createMaterial(vertexShader, fragmentShader, customUniforms, useWebGL2, useLogarithmicDepth) {
        ...
        const shaderAtlas = [...Array(16).keys()].map(i => new THREE$1$1.Vector4());
        if (this.atlas) {
            for (let i = 0; i < this.atlas.getFrameSetCount(); i++) {
                const frameSet = this.atlas.getFrameSet(i);
                shaderAtlas[i] = new THREE$1$1.Vector4(frameSet.x, frameSet.y, frameSet.width, frameSet.height);
            }
        }
        ...
        return new THREE$1$1.ShaderMaterial({
            uniforms: customUniforms,
            vertexShader,
            fragmentShader,
            transparent: true,
            alphaTest: 1.0,
            blending: THREE$1$1.NormalBlending,
            blendDstAlpha: THREE$1$1.OneMinusSrcAlphaFactor,
            blendSrcAlpha: THREE$1$1.SrcAlphaFactor,
            depthTest: true,
            depthWrite: false
        } )
    ...
    static fromJSON(params) {
        ...
        const atlasTexture = new THREE$1$1.TextureLoader().load(atlasJSON.texturePath);
        ...
        if (params.blending == 'Additive') {
            renderer.blending = THREE$1$1.AdditiveBlending;
        } else {
            renderer.blending = THREE$1$1.NormalBlending;
        }
        return renderer
    }
    ...
    toJSON(texturePathGenerator) {
        ...
        let blending = 'Normal';
        if (this.material.blending === THREE$1$1.AdditiveBlending) {
            blending = 'Additive';
        }
...
class BuiltinType {
    ...
    static getTypeID(type) {
        ...
        if (type === THREE$1$1.Vector2) {
            typeID = BuiltinType.Vector2;
        } else if (type === THREE$1$1.Vector3) {
            typeID = BuiltinType.Vector3;
        } else if (type === THREE$1$1.Vector4) {
            typeID = BuiltinType.Vector4;
        } else if (type === THREE$1$1.Color) {
    ...
    static loadJSONParameter(param, type) {
        switch (type) {
            case THREE$1$1.Vector2:
                return new THREE$1$1.Vector2().fromArray(param)
            case THREE$1$1.Vector3:
                return new THREE$1$1.Vector3().fromArray(param)
            case THREE$1$1.Vector4:
                return new THREE$1$1.Vector4().fromArray(param)
            case THREE$1$1.Color:
                return new THREE$1$1.Color().fromArray(param)
        }
...
class RandomGenerator extends Generator {
    ...
    generate(out) {
        ...
        switch (this.outTypeID) {
            case BuiltinType.Default:
                ...
                if (this.normalize) out = out < 0 ? -1 : 1.0;
            break
    ...
    static getParameterJSON(param) {
        if (param instanceof THREE$1$1.Vector2 || param instanceof THREE$1$1.Vector3 ||
            param instanceof THREE$1$1.Vector4 || param instanceof THREE$1$1.Color) return param.toArray()
        return param
    }
...
class BoxPositionInitializer extends ParticleStateInitializer {
    ...
    constructor(range, offset) {
        super();
        this.randomGenerator = new RandomGenerator(THREE$1$1.Vector3, range, offset, 0.0, 0.0, false);
    }
    ...
    static fromJSON(particleSystem, params) {
        return new BoxPositionInitializer(new THREE$1$1.Vector3().fromArray(params.range),
                                          new THREE$1$1.Vector3().fromArray(params.offset))
    }
...
class RandomVelocityInitializer extends ParticleStateInitializer {
    ...
    constructor(directionRange, directionOffset, speedRange, speedOffset, normalizeDirection = true) {
        super();
        this.directionGenerator = new RandomGenerator(THREE$1$1.Vector3, directionRange, directionOffset, 0.0, 0.0, true);
    ...
    static fromJSON(particleSystem, params) {
        return new RandomVelocityInitializer(new THREE$1$1.Vector3().fromArray(params.range),
                                             new THREE$1$1.Vector3().fromArray(params.offset),
...
class ContinuousArray {
    ...
    * [Symbol.iterator]() {
    ...
    getInterpolatorForTypeID = function() {
        ...
        const upper2 = new THREE$1$1.Vector2();
        const upper3 = new THREE$1$1.Vector3();
        const upper4 = new THREE$1$1.Vector4();
        const upperC = new THREE$1$1.Color();
...
class SizeInterpolatorOperator extends InterpolatorOperator {
    ...
    constructor(relativeToInitialValue = false) {
        super(THREE$1$1.Vector2, relativeToInitialValue);
    }

    addElementsFromParameters(elementParametersTValuePairs) {
        super.addElementsFromElementClassAndParameters(THREE$1$1.Vector2, elementParametersTValuePairs);
    }

    updateState = function() {

        const tempSize = new THREE$1$1.Vector2();
...
class ColorInterpolatorOperator extends InterpolatorOperator {
    ...
    constructor(relativeToInitialValue = false) {
        super(THREE$1$1.Color, relativeToInitialValue);
    }

    addElementsFromParameters(elementParametersTValuePairs) {
        super.addElementsFromElementClassAndParameters(THREE$1$1.Color, elementParametersTValuePairs);
    }

    updateState = function() {

        const tempColor = new THREE$1$1.Color();
...
class SphereRandomGenerator extends Generator {
    ...
    constructor(rangeTheta, offsetTheta, rangePhi, offsetPhi, rangeRadius, offsetRadius,
        ...
        this.tempUp = new THREE$1$1.Vector3().set(0.0, 1.0, 0.0);
    }
...
class FlickerLight extends Component {
    ...
    constructor(parent, intensity, intensityFlux, color, distance, decay, shadows = undefined) {
        ...
        this.lastPositionAdjuster = new THREE$1$1.Vector3();
        this.positionAdjuster = new THREE$1$1.Vector3();
        this.intensity = 1.0;
    ...
    init(parent, intensity, intensityFlux, color, distance, decay, shadows = undefined) {
        this.owner = new THREE$1$1.Object3D();
        parent.add(this.owner);

        color = color || new THREE$1$1.Color();
        distance = distance || 0;
        if (decay == null || decay == undefined) decay = 2.0;
        const shadowsEnabled = !!shadows;

        this.light = new THREE$1$1.PointLight(0xffffff, 2, 0, 1);
...
const Photons = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
   __proto__: null,
...
const _sfc_main$1 = /* @__PURE__ */ _defineComponent$1({
  ...
  setup(__props) {
    const props = __props;
    const PhotonsManager = new Manager();
    const jsonTypeStore = new JSONTypeStore();
    jsonTypeStore.addNamespace("THREE", THREE$1);
    jsonTypeStore.addNamespace("Photons", Photons);
    const { renderer, camera, scene } = Fs();
    const setupEmbers = (scale, position) => {
      const embersRoot = new THREE$1.Object3D();
      embersRoot.position.copy(position);
      const texturePath = "./plugins/digitalCity/image/fireF/ember.png";
      const embersTexture = new THREE$1.TextureLoader().load(texturePath);
      const embersAtlas = new Atlas(embersTexture, texturePath);
      embersAtlas.addFrameSet(1, 0, 0, 1, 1);
      const embersRenderer = new AnimatedSpriteRenderer(true, embersAtlas, true, THREE$1.AdditiveBlending);
      const embersParticleSystem = new ParticleSystem(embersRoot, embersRenderer, renderer);
      embersParticleSystem.init(150);
      embersParticleSystem.setEmitter(new ConstantParticleEmitter(6));
      const sizeInitializerGenerator = new RandomGenerator(
        THREE$1.Vector2,
        new THREE$1.Vector2(0, 0),
        new THREE$1.Vector2(scale * 0.15, scale * 0.15),
        0,
        0,
        false
      );
      embersParticleSystem.addParticleStateInitializer(new LifetimeInitializer(3, 1, 0, 0, false));
      embersParticleSystem.addParticleStateInitializer(new SizeInitializer(sizeInitializerGenerator));
      embersParticleSystem.addParticleStateInitializer(
        new BoxPositionInitializer(new THREE$1.Vector3(0.05 * scale, 0, 0.05 * scale), new THREE$1.Vector3(-0.025 * scale, 0, -0.025 * scale))
      );
      embersParticleSystem.addParticleStateInitializer(
        new RandomVelocityInitializer(
          new THREE$1.Vector3(0.4 * scale, 0.5 * scale, 0.4 * scale),
          new THREE$1.Vector3(-0.2 * scale, 0.8 * scale, -0.2 * scale),
          0.6 * scale,
          0.8 * scale,
          false
        )
      );
      const embersOpacityOperator = embersParticleSystem.addParticleStateOperator(new OpacityInterpolatorOperator());
      ...
      const embersColorOperator = embersParticleSystem.addParticleStateOperator(new ColorInterpolatorOperator(true));
      embersColorOperator.addElementsFromParameters([
        [[1, 0.7, 0], 0],
        [[1, 0.6, 0], 0.5],
        [[1, 0.4, 0], 1]
      ]);
      const acceleratorOperatorGenerator = new SphereRandomGenerator(
        Math.PI * 2,
        0,
        Math.PI,
        -Math.PI / 2,
      ...
      embersParticleSystem.addParticleStateOperator(new AccelerationOperator(acceleratorOperatorGenerator));
      embersParticleSystem.setSimulateInWorldSpace(true);
      embersParticleSystem.start();
      return embersParticleSystem;
    };
    const setupBaseFlame = (scale, position) => {
      const baseFlameRoot = new THREE$1.Object3D();
      baseFlameRoot.position.copy(position);
      const texturePath = "./plugins/digitalCity/image/fireF/base_flame.png";
      const baseFlameTexture = new THREE$1.TextureLoader().load(texturePath);
      const baseFlameAtlas = new Atlas(baseFlameTexture, texturePath);
      baseFlameAtlas.addFrameSet(18, 0, 0, 128 / 1024, 128 / 512);
      const baseFlameRenderer = new AnimatedSpriteRenderer(true, baseFlameAtlas, true);
      const baseFlameParticleSystem = new ParticleSystem(baseFlameRoot, baseFlameRenderer, renderer);
      baseFlameParticleSystem.init(50);
      baseFlameParticleSystem.setEmitter(new ConstantParticleEmitter(10));
      ...
      baseFlameParticleSystem.addParticleStateInitializer(new LifetimeInitializer(0, 0, 0, 0, false));
      baseFlameParticleSystem.addParticleStateInitializer(
        new RotationInitializer(new RandomGenerator(0, Math.PI / 2, -Math.PI / 2, 0, 0, false))
      );
      baseFlameParticleSystem.addParticleStateInitializer(new RotationalSpeedInitializer(1, -1, 0, 0, false));
      baseFlameParticleSystem.addParticleStateInitializer(
        new SizeInitializer(
          new RandomGenerator(
            THREE$1.Vector2,
            new THREE$1.Vector2(0.25 * scale, 0.25 * scale),
            new THREE$1.Vector2(0.5 * scale, 0.5 * scale),
            0,
            0,
            false
          )
      ...
      baseFlameParticleSystem.addParticleStateInitializer(
        new RandomVelocityInitializer(
          new THREE$1.Vector3(0.05 * scale, 0.4 * scale, 0.05 * scale),
          new THREE$1.Vector3(-0.025 * scale, 0.8 * scale, -0.025 * scale),
          0.35 * scale,
          0.5 * scale,
          false
        )
      );
      baseFlameParticleSystem.addParticleStateInitializer(new SequenceInitializer(baseFlameParticleSequences));
      baseFlameParticleSystem.addParticleStateOperator(new SequenceOperator(baseFlameParticleSequences, 0.07, false));
      const baseFlameOpacityOperator = baseFlameParticleSystem.addParticleStateOperator(new OpacityInterpolatorOperator());
      ...
      const baseFlameSizeOperator = baseFlameParticleSystem.addParticleStateOperator(new SizeInterpolatorOperator(true));
      baseFlameSizeOperator.addElementsFromParameters([
        [[0.6, 0.6], 0],
        [[1, 1], 0.4],
        [[1, 1], 1]
      ]);
      const baseFlameColorOperator = baseFlameParticleSystem.addParticleStateOperator(new ColorInterpolatorOperator(true));
      ...
      baseFlameParticleSystem.addParticleStateOperator(
        new AccelerationOperator(
          new RandomGenerator(THREE$1.Vector3, new THREE$1.Vector3(0, 0, 0), new THREE$1.Vector3(0, 1.5 * scale, 0), 0, 0, false)
        )
      );
    ...
    const setupBrightFLame = (scale, position) => {
      const brightFlameRoot = new THREE$1.Object3D();
      brightFlameRoot.position.copy(position);
      const texturePath = "./plugins/digitalCity/image/fireF/bright_flame.png";
      const brightFlameTexture = new THREE$1.TextureLoader().load(texturePath);
      const brightFlameAtlas = new Atlas(brightFlameTexture, texturePath);
      brightFlameAtlas.addFrameSet(16, 0, 0, 212 / 1024, 256 / 1024);
      const brightFlameRenderer = new AnimatedSpriteRenderer(true, brightFlameAtlas, true);
      const brightFlameParticleSystem = new ParticleSystem(brightFlameRoot, brightFlameRenderer, renderer);
      brightFlameParticleSystem.init(20);
      brightFlameParticleSystem.setEmitter(new ConstantParticleEmitter(5));
      ...
      brightFlameParticleSystem.addParticleStateInitializer(new LifetimeInitializer(0, 0, 0, 0, false));
      brightFlameParticleSystem.addParticleStateInitializer(
        new RotationInitializer(new RandomGenerator(0, Math.PI, -Math.PI / 2, 0, 0, false))
      );
      brightFlameParticleSystem.addParticleStateInitializer(new RotationalSpeedInitializer(Math.PI / 2, -Math.PI / 4, 0, 0, false));
      brightFlameParticleSystem.addParticleStateInitializer(
        new SizeInitializer(
          new RandomGenerator(THREE$1.Vector2, new THREE$1.Vector2(0, 0), new THREE$1.Vector2(0, 0), 0.2 * scale, 0.65 * scale, false)
        )
      );
      brightFlameParticleSystem.addParticleStateInitializer(
        new BoxPositionInitializer(new THREE$1.Vector3(0.1 * scale, 0, 0.1 * scale), new THREE$1.Vector3(-0.05 * scale, 0, -0.05 * scale))
      );
      brightFlameParticleSystem.addParticleStateInitializer(
        new RandomVelocityInitializer(
          new THREE$1.Vector3(0.02 * scale, 0.4 * scale, 0.02 * scale),
          new THREE$1.Vector3(-0.01 * scale, 0.4 * scale, -0.01 * scale),
          0.1 * scale,
          0.2 * scale,
          false
        )
      );
      brightFlameParticleSystem.addParticleStateInitializer(new SequenceInitializer(brightFlameParticleSequences));
      brightFlameParticleSystem.addParticleStateOperator(new SequenceOperator(brightFlameParticleSequences, 0.1, false));
      const brightFlameOpacityOperator = brightFlameParticleSystem.addParticleStateOperator(new OpacityInterpolatorOperator());
      ...
      const brightFlameSizeOperator = brightFlameParticleSystem.addParticleStateOperator(new SizeInterpolatorOperator(true));
      ...
      const brightFlameColorOperator = brightFlameParticleSystem.addParticleStateOperator(new ColorInterpolatorOperator(true));
    ...
    const setupLights = (position, intensity = 10) => {
      const lightParent = new THREE$1.Object3D();
      ...
      return new FlickerLight(lightParent, intensity, 2, new THREE$1.Color().setRGB(1, 0.8, 0.4), 0, 1, flickerLightShadows);
    };
    const flamePosition2 = new THREE$1.Vector3(...props.position);
...
//# sourceMappingURL=fireF.DDa52KNw1767105581793.js.map
